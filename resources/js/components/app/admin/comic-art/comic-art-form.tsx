...
export default function ComicArtForm({ comicArt }: ComicArtFormProps) {
    const [existingImages, setExistingImages] = useState(
        comicArt?.images.map((img) => ({
            id: img.id,
            url: `${img.image}`,
            order: img.order,
        })) || [],
    ...
    const { data, setData, post, put, processing, errors, reset, transform } = useForm({
        title: comicArt?.title || '',
        description: comicArt?.description || '',
        images: comicArt?.images?.map((comicArt) => comicArt?.image) || ([] as string[]),
        new_images: [],
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const url = comicArt ? route('comic-art.update', comicArt.id) : route('comic-art.store');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
                <label htmlFor="title" className="block text-sm font-medium">
                    Comic Title <span className="text-red-500">*</span>
                </label>
                <Input id="title" placeholder="Enter comic title" value={data.title} onChange={(e) => setData('title', e.target.value)} />
