...
export const createStickerArtColumns = ({ handleDeleteClick }: StickerArtColumnsPropsT): ColumnDef<StickerArtT>[] => [
    {
        accessorKey: 'title',
        header: 'Title',
    },
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div className="flex justify-center gap-2">
                <Link href={route('sticker-art.show', row.original.id)}>
                    <Button variant="ghost" size="icon">
                        <Eye className="h-4 w-4" />
                    </Button>
                </Link>
                <Link href={route('sticker-art.edit', row.original.id)}>
                    <Button variant="ghost" size="icon">
